<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                ...
                <h1 class="text-xl sm:text-2xl font-light text-white">Historial Primera Oferta</h1>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-400 mt-1">Registro completo de todas las ejecuciones de actualización de primera oferta</p>
                            </div>
                        </div>
            ...
            <div class="bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-700/50 overflow-hidden mb-6">
                <div class="p-6 sm:p-8">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        @if($filtroRapido ?? false || $fechaDesde ?? false || $fechaHasta ?? false || $horaDesde ?? false || $horaHasta ?? false || $busqueda ?? false)
                            ...
                            <div class="bg-gray-700/50 rounded-lg p-4">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <button type="button" onclick="aplicarFiltroRapido('hoy')" 
                                            class="px-3 py-1.5 text-xs font-medium rounded-md transition-colors {{ ($filtroRapido ?? 'hoy') === 'hoy' ? 'bg-blue-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500 border border-gray-500' }}">
                                        🟢 Hoy
                                    </button>
                                    <button type="button" onclick="aplicarFiltroRapido('ayer')" 
                                            class="px-3 py-1.5 text-xs font-medium rounded-md transition-colors {{ ($filtroRapido ?? 'hoy') === 'ayer' ? 'bg-blue-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500 border border-gray-500' }}">
                                        📅 Ayer
                                    </button>
                                    <button type="button" onclick="aplicarFiltroRapido('7dias')" 
                                            class="px-3 py-1.5 text-xs font-medium rounded-md transition-colors {{ ($filtroRapido ?? 'hoy') === '7dias' ? 'bg-blue-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500 border border-gray-500' }}">
                                        📊 Últimos 7 días
                                    </button>
                                    <button type="button" onclick="aplicarFiltroRapido('30dias')" 
                                            class="px-3 py-1.5 text-xs font-medium rounded-md transition-colors {{ ($filtroRapido ?? 'hoy') === '30dias' ? 'bg-blue-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500 border border-gray-500' }}">
                                        📈 Últimos 30 días
                                    </button>
                                    <button type="button" onclick="aplicarFiltroRapido('90dias')" 
                                            class="px-3 py-1.5 text-xs font-medium rounded-md transition-colors {{ ($filtroRapido ?? 'hoy') === '90dias' ? 'bg-blue-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500 border border-gray-500' }}">
                                        📊 Últimos 90 días
                                    </button>
                                    <button type="button" onclick="aplicarFiltroRapido('180dias')" 
                                            class="px-3 py-1.5 text-xs font-medium rounded-md transition-colors {{ ($filtroRapido ?? 'hoy') === '180dias' ? 'bg-blue-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500 border border-gray-500' }}">
                                        📈 Últimos 180 días
                                    </button>
                                    <button type="button" onclick="aplicarFiltroRapido('1año')" 
                                            class="px-3 py-1.5 text-xs font-medium rounded-md transition-colors {{ ($filtroRapido ?? 'hoy') === '1año' ? 'bg-blue-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500 border border-gray-500' }}">
                                        📅 Último año
                                    </button>
                                    <button type="button" onclick="aplicarFiltroRapido('siempre')" 
                                            class="px-3 py-1.5 text-xs font-medium rounded-md transition-colors {{ ($filtroRapido ?? 'hoy') === 'siempre' ? 'bg-blue-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500 border border-gray-500' }}">
                                        🌍 Desde siempre
                                    </button>
                                </div>
                            </div>
                ...
                <div class="p-6 sm:p-8">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div class="bg-gradient-to-br from-blue-900/20 to-blue-800/20 p-4 rounded-xl border border-blue-700/50 text-center">
                            <div class="text-3xl font-bold text-blue-400 mb-2">{{ number_format($totalEjecuciones ?? 0) }}</div>
                        ...
                        <div class="bg-gradient-to-br from-purple-900/20 to-purple-800/20 p-4 rounded-xl border border-purple-700/50 text-center">
                            <div class="text-3xl font-bold text-purple-400 mb-2">{{ number_format($totalOfertas ?? 0) }}</div>
                        ...
                        <div class="bg-gradient-to-br from-green-900/20 to-emerald-800/20 p-4 rounded-xl border border-green-700/50 text-center">
                            <div class="text-3xl font-bold text-green-400 mb-2">{{ number_format($totalActualizadas ?? 0) }}</div>
                        ...
                        <div class="bg-gradient-to-br from-red-900/20 to-red-800/20 p-4 rounded-xl border border-red-700/50 text-center">
                            <a href="#errores-por-tienda" class="block hover:scale-105 transition-transform duration-200 cursor-pointer">
                                <div class="text-3xl font-bold text-red-400 mb-2">{{ number_format($totalErrores ?? 0) }}</div>
                                <div class="text-sm text-red-300 font-medium">Total Errores</div>
                                @if(($totalOfertas ?? 0) > 0)
                                    <div class="text-xs text-red-200 mt-1">
                                        {{ number_format((($totalErrores ?? 0) / ($totalOfertas ?? 1)) * 100, 1) }}% del total
                                    </div>
                                @endif
                            </a>
                        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                <div class="bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-700/50 overflow-hidden">
                    <div class="p-6 sm:p-8">
                        <h3 class="text-lg sm:text-xl font-bold text-white mb-4">📈 Estadísticas por Hora del Día</h3>
            ...
            <div id="errores-por-tienda" class="bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-700/50 overflow-hidden mb-6">
                <div class="p-6 sm:p-8">
                    <h3 class="text-lg sm:text-xl font-bold text-white mb-6">🚨 Errores por Tienda</h3>
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="border-b border-gray-700/50">
                                        ...
                                        <th class="text-center py-3 px-4 font-semibold text-gray-300">Resueltas</th>
                                ...
                                <tbody class="divide-y divide-gray-700/50">
                                    @foreach($erroresPorTienda ?? [] as $tienda => $errores)
                                        @php
                                            $porcentaje = ($totalErrores ?? 0) > 0 ? ($errores / ($totalErrores ?? 1)) * 100 : 0;
                                        @endphp
                                        <tr class="hover:bg-gray-700/50 transition-colors">
                                            ...
                                            <td class="py-3 px-4 text-center">
                                                @php
                                                    $resueltas = $urlsResueltasPorTienda[$tienda] ?? 0;
                                                    $porcentajeResueltas = $errores > 0 ? ($resueltas / $errores) * 100 : 0;
                                                @endphp
                                                <div class="text-lg font-bold text-green-400">
                                                    {{ number_format($resueltas) }}
                                                    @if($errores > 0)
                                                        <span class="text-xs text-green-300 ml-1">
                                                            ({{ number_format($porcentajeResueltas, 1) }}%)
                                                        </span>
                                                    @endif
                                                </div>
                                            </td>
                                            ...
                                            <td class="py-3 px-4">
                                                <div class="w-full bg-gray-700 rounded-full h-3">
                                                    <div class="bg-gradient-to-br from-red-500 to-pink-600 h-3 rounded-full transition-all duration-300" 
            ...
            <div class="bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-700/50 overflow-hidden">
                ...
                @else
                    <div class="p-12 sm:p-16 text-center">
                        ...
                        <p class="text-gray-400 max-w-md mx-auto">Aún no se han realizado ejecuciones de actualización de primera oferta. Cuando ejecutes el proceso, aquí aparecerá el historial completo.</p>
                    </div>
                @endif
            </div>
        </div>
    ...
    <script>
        function verDetalles(ejecucionId) {
            ...
            fetch(`{{ route('admin.scraping.actualizar-primera-oferta.detalles', '') }}/${ejecucionId}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        mostrarDetallesEjecucion(data.ejecucion);
                    } else {
                        mostrarError('Error al cargar los detalles: ' + (data.error || 'Error desconocido'));
                    }
                })
                .catch(error => {
        ...
        function mostrarDetallesEjecucion(ejecucion) {
            const log = ejecucion.log || {};
            const resultados = log.resultados || [];
                ...
                <div class="space-y-8">
                    ...
                    <div class="bg-gradient-to-br from-blue-900/20 to-indigo-800/20 p-6 rounded-2xl border border-blue-700/50">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="text-blue-300">#${ejecucion.id}</span>
                            </div>
                            <div class="flex items-center space-x-2">
                                <span class="font-semibold text-blue-200">Estado:</span>
                                <span class="px-3 py-1 text-xs font-bold rounded-full ${log.estado === 'completada' ? 'bg-green-900 text-green-200' : 'bg-yellow-900 text-yellow-200'}">${log.estado || 'N/A'}</span>
                            </div>
                            <div class="flex items-center space-x-2">
                                <span class="font-semibold text-blue-200">Inicio:</span>
                                <span class="text-blue-300">${new Date(ejecucion.inicio).toLocaleString('es-ES')}</span>
                            </div>
                            ${ejecucion.fin ? `
                            <div class="flex items-center space-x-2">
                                <span class="font-semibold text-blue-200">Fin:</span>
                                <span class="text-blue-300">${new Date(ejecucion.fin).toLocaleString('es-ES')}</span>
                            </div>
                            ` : ''}
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                        <div class="bg-gradient-to-br from-gray-700 to-gray-800 p-6 rounded-2xl border border-gray-600/50 text-center">
                            <div class="text-3xl font-bold text-white mb-2">${log.total_ofertas || 0}</div>
                        ...
                        <div class="bg-gradient-to-br from-green-900/20 to-emerald-800/20 p-6 rounded-2xl border border-green-700/50 text-center">
                            <div class="text-3xl font-bold text-green-400 mb-2">${log.actualizadas || 0}</div>
                            <div class="text-sm text-green-300 font-medium">Actualizadas</div>
                        </div>
                        <div class="bg-gradient-to-br from-red-900/20 to-red-800/20 p-6 rounded-2xl border border-red-700/50 text-center">
                            <div class="text-3xl font-bold text-red-400 mb-2">${log.errores || 0}</div>
                            <div class="text-sm text-red-300 font-medium">Errores</div>
                        </div>
                        <div class="bg-gradient-to-br from-purple-900/20 to-purple-800/20 p-6 rounded-2xl border border-purple-700/50 text-center">
                            <div class="text-3xl font-bold text-purple-400 mb-2">${log.procesadas || 0}</div>
                            <div class="text-sm text-purple-300 font-medium">Procesadas</div>
                        </div>
                    </div>
            `;
            ...
            if (resultados.length > 0) {
                    ...
                    <div class="bg-gray-800 border border-gray-700/50 rounded-2xl overflow-hidden shadow-lg">
                        <div class="px-6 py-4 border-b border-gray-700/50 bg-gradient-to-r from-gray-700 to-gray-800">
                            <h3 class="font-bold text-white flex items-center">
                                ...
                                Resultados Detallados (${resultados.length} ofertas)
                            </h3>
                        </div>
                ...
                resultados.forEach((resultado, index) => {
                    const esExito = resultado.success;
                            ...
                            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                                <div class="flex-1 space-y-3">
                                    <div class="flex items-center space-x-3">
                                        <span class="text-sm font-bold text-white">Oferta #${resultado.oferta_id || 'N/A'}</span>
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <span class="text-gray-400">${resultado.tienda_nombre || 'N/A'}</span>
                                        </div>
                                        <div class="flex items-center space-x-2">
                                            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <a href="${resultado.url || '#'}" target="_blank" class="text-blue-400 hover:text-blue-300 underline truncate">${resultado.url || 'N/A'}</a>
                                        </div>
                                    </div>
                    `;
                    ...
                    if (esExito) {
                        contenido += `
                            <div class="flex items-center space-x-2 text-sm">
                                ...
                                <span class="line-through text-gray-500">€${resultado.precio_anterior || 'N/A'}</span> 
                                <svg class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path>
                                </svg>
                                <span class="text-green-400 font-bold">€${resultado.precio_nuevo || 'N/A'}</span>
                            </div>
                        `;
                    } else {
                        contenido += `
                            <div class="flex items-center space-x-2 text-sm">
                                ...
                                <span class="text-red-400">${resultado.error || 'Error desconocido'}</span>
                            </div>
                        `;
                    }
        ...
        function verErroresTienda(tienda) {
            ...
            const horaDesde = urlParams.get('hora_desde') || '';
            const horaHasta = urlParams.get('hora_hasta') || '';
            
            // Construir URL para la petición
            const url = new URL('{{ route("admin.scraping.actualizar-primera-oferta.errores.tienda") }}', window.location.origin);
            url.searchParams.set('tienda', tienda);
            if (filtroRapido) url.searchParams.set('filtro_rapido', filtroRapido);
