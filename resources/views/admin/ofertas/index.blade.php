<x-app-layout>
    ...
    <div class="py-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        @php
            ...
            $columnasEspecTabla = $columnasEspecTabla ?? $columnasEspecListado;
            $columnaFusionadaUnidades = $columnaFusionadaUnidades ?? null;
            $textoColumnaUnidadesPorOfertaId = $textoColumnaUnidadesPorOfertaId ?? [];
            ...
            $numColEspec = count($columnasEspecTabla);
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        @foreach ($columnasEspecTabla as $colEspec)
                        ...
                        @unless($ocultarUnidadesYPrecioUnidad)
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Unidades @if($columnaFusionadaUnidades) / {{ $columnaFusionadaUnidades['texto'] }}@endif
                            </th>
                        @endunless
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($ofertas as $oferta)
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-300 transition-colors {{ $oferta->mostrar === 'no' ? 'opacity-60 bg-gray-50' : '' }}">
                            ...
                            @foreach ($columnasEspecTabla as $idx => $colEspec)
                                <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-200 max-w-xs">{{ ($celdasEspecPorOfertaId[$oferta->id] ?? [])[$idx] ?? '—' }}</td>
                            @endforeach
                            @unless($ocultarUnidadesYPrecioUnidad)
                                <td class="px-6 py-4">
                                    <div>{{ $oferta->unidades }}</div>
                                    @if($columnaFusionadaUnidades)
                                        @php $textoColUnidades = $textoColumnaUnidadesPorOfertaId[$oferta->id] ?? '—'; @endphp
                                        @if($textoColUnidades !== '—')
                                            <div class="border-t border-gray-200 mt-1 pt-1 text-sm text-gray-600 dark:text-gray-300">
                                                <span class="font-medium">{{ $columnaFusionadaUnidades['texto'] }}:</span> {{ $textoColUnidades }}
                                            </div>
                                        @endif
                                    @endif
                                </td>
