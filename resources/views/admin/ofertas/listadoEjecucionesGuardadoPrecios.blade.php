<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                Historial de ejecuciones de ofertas
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-4 flex justify-between items-center">
            ...
            <form method="POST" action="{{ route('admin.ofertas.ejecuciones.eliminar.antiguas') }}" class="flex items-center gap-2 mb-4">
                @csrf
                <label class="text-sm text-gray-700 dark:text-white">Eliminar las</label>
                <input type="number" name="cantidad" min="1" required class="border px-3 py-2 rounded w-24 text-sm">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end items-center space-x-6">
                                ...
                                <form method="POST" action="{{ route('admin.ejecuciones.eliminar', $ejecucion) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('¿Eliminar esta ejecución?')"
    ...
    <script>
        async function verLog(id) {
            id = parseInt(id);
            const response = await fetch(`/panel-privado/ofertas/historico-precios/ejecuciones/${id}/log`);
