...
class YougetController extends PlantillaTiendaController
{
    /**
    ...
    public function obtenerPrecio($url, $variante = null, $tienda = null, $oferta = null): JsonResponse
    {
        ...
        if ($this->esProductoYaNoDisponible($html)) {
            ...
            return response()->json(['success' => false, 'error' => 'Producto no disponible']);
        }
        ...
        if ($precio === null) {
            return response()->json([
                'success' => false,
                'error'   => 'No se pudo encontrar el precio en la pagina de Youget',
            ]);
        }
    ...
    /**
     * Ej.: <li>Este producto ya no esta disponible.</li>
     */
    private function esProductoYaNoDisponible(string $html): bool
    {
        return strpos($html, 'Este producto ya no esta disponible') !== false
            || strpos($html, 'Este producto ya no está disponible') !== false;
    }
     ...
     * <span class="current-price-value" content="639.9">639,90 €</span>
     *
     * Importante: pedimos etiqueta <span ...> real para evitar capturar
     * referencias JS como "priceSel: '.current-price-value'".
     */
    private function extraerPrecioDesdeCurrentPriceValue(string $html): ?float
    {
        ...
        // Fallback si cambian el atributo content y solo queda texto visible.
        if (
            preg_match(
    ...
    /**
     * Normaliza "639.9", "639,90" o "1.234,56" a float.
     */
    ...
    public function extraerProductosYSiguientePagina(string $html, string $urlPeticionActual): array
    {
        ...
        // Fallback en titulo del producto:
        // <h3><a href="..." class="product_name">...</a></h3>
        if (
            preg_match_all(
                '~<h3[^>]*>\s*<a[^>]*\bhref=(["\'])(?<u>[^"\']+)\1[^>]*\bclass=(["\'])[^"\']*\bproduct_name\b[^"\']*\3~i',
                $html,
                $mTitle
            )
        ) {
        ...
        // Regla solicitada: avanzar page+1 hasta que una pagina no tenga productos.
        $siguienteUrl = null;
